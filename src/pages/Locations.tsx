import { ArrowRight, BadgePercent, Clock, Coffee, Heart, MapPin, Navigation, Phone, Quote, Share2, Star } from 'lucide-react';
...
export function Locations() {
  ...
  return <div className="min-h-screen bg-[#f7f1e8] pb-28 overflow-x-hidden">
    ...
    <div className="px-6">
      ...
      {(story?.title || story?.body || reviews.length > 0) && <div className="mt-8 space-y-3 text-center">{story?.title && <h2 className="font-serif italic text-2xl leading-8 text-neutral-950">{story.title}</h2>}{story?.body && <p className="font-serif text-[15px] leading-7 text-neutral-500 max-w-[320px] mx-auto">{story.body}</p>}<CustomerReviewsMarquee reviews={reviews} rating={rating?.value} reviewCount={rating?.reviewCount} /></div>}
      ...
      {cafeDetails.amenities?.length > 0 && <div className="rounded-3xl bg-white border border-neutral-100 shadow-[0_14px_30px_rgba(74,49,35,0.06)] mt-6 p-4"><div className="grid grid-cols-5 gap-2">{cafeDetails.amenities.map((amenity) => <div key={amenity} className="flex flex-col items-center gap-1.5 text-center"><Coffee className="size-4 text-[#7a4f2a]" /><span className="text-[10px] leading-3 text-neutral-500">{amenity}</span></div>)}</div></div>}
