...
export function Locations() {
  ...
  return <div className="min-h-screen bg-[var(--color-background)] pb-28 overflow-x-hidden">
    ...
    <div className="px-6">
      <div className="pt-5 space-y-1.5"><h1 className="font-semibold text-neutral-950 text-xl leading-7">{cafeName}</h1>{contact?.address && <div className="text-neutral-500 text-sm leading-5 flex items-center gap-1.5"><MapPin className="size-3.5 text-[var(--color-primary)]" /><span>{contact.address}</span></div>}</div>
      {(rating?.value !== undefined || rating?.reviewCount !== undefined) && <div className="flex justify-between items-center mt-3 gap-3"><div className="text-neutral-950 text-sm leading-5 flex items-center gap-1.5"><Star className="size-4 fill-[var(--color-primary)] text-[var(--color-primary)]" /><span className="font-medium">{rating.value}</span><span className="text-neutral-500">({rating.reviewCount} reviews)</span></div>{hours?.[today as keyof typeof hours] && <div className="shadow-sm rounded-full bg-white border border-neutral-200 px-3 py-1.5 shrink-0"><div className="text-neutral-950 text-xs leading-4 flex items-center gap-1.5 whitespace-nowrap"><span className="size-2 rounded-full bg-emerald-500" /><span>Today • {hours[today as keyof typeof hours]}</span></div></div>}</div>}
      ...
      <div className={`shadow-[0_18px_40px_rgba(74,49,35,0.08)] rounded-3xl bg-white border border-neutral-100 mt-5 p-4 grid ${quickActionColumns} gap-2`}>
        {location?.googleMapsUrl && <button onClick={() => openExternal(location.googleMapsUrl)} className="rounded-2xl bg-[var(--color-surface-subtle)] text-[var(--color-text)] px-2 py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform"><Navigation className="size-5 text-[var(--color-primary)]" /><span className="font-medium text-[11px] leading-3.5 text-center">Directions</span></button>}
        {contact?.phone && <a href={`tel:${contact.phone}`} className="rounded-2xl bg-[var(--color-surface-subtle)] text-[var(--color-text)] px-2 py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform"><Phone className="size-5 text-[var(--color-primary)]" /><span className="font-medium text-[11px] leading-3.5 text-center">Call Now</span></a>}
        {cafeDetails.links?.instagram && <button onClick={() => openExternal(cafeDetails.links.instagram)} className="rounded-2xl bg-[var(--color-surface-subtle)] text-[var(--color-text)] px-2 py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform"><Instagram className="size-5 text-[var(--color-primary)]" /><span className="font-medium text-[11px] leading-3.5 text-center">Instagram</span></button>}
        {cafeDetails.links?.googleReviews && <button onClick={() => openExternal(cafeDetails.links.googleReviews)} className="rounded-2xl bg-[var(--color-surface-subtle)] text-[var(--color-text)] px-2 py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform"><Star className="size-5 text-[var(--color-primary)]" /><span className="font-medium text-[11px] leading-3.5 text-center">Rate Us</span></button>}
        {offer?.title && <button onClick={() => offer.buttonUrl && openExternal(offer.buttonUrl)} className="rounded-2xl bg-[var(--color-surface-subtle)] text-[var(--color-text)] px-2 py-3 flex flex-col items-center gap-1.5 active:scale-95 transition-transform"><BadgePercent className="size-5 text-[var(--color-primary)]" /><span className="font-medium text-[11px] leading-3.5 text-center">{offer.buttonText || "Today's Offer"}</span></button>}
      </div>
