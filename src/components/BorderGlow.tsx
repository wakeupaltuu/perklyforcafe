...
interface BorderGlowProps {
  ...
  colors?: string[];
  fillOpacity?: number;
  tapGlow?: boolean; // ✅ NEW: enable glow on tap for mobile
}
...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  fillOpacity = 0.5,
  tapGlow = false, // ✅ NEW: default false
}) => {
  const cardRef = useRef<HTMLDivElement>(null);
  const [isHovered, setIsHovered] = useState(false);
  const [tapActive, setTapActive] = useState(false); // ✅ NEW: tap state
  const [cursorAngle, setCursorAngle] = useState(45);
  const [edgeProximity, setEdgeProximity] = useState(0);
  const [sweepActive, setSweepActive] = useState(false);

  // ✅ NEW: Handle tap for mobile
  const handleTap = () => {
    if (tapGlow) {
      setTapActive(true);
  ...
  // ✅ NEW: include tapActive in visibility
  const isVisible = isHovered || sweepActive || tapActive;
  ...
  return (
    <div
      ...
      onClick={handleTap} // ✅ NEW: tap to trigger glow
