...
export function RewardRedemptionSheet({ reward, cafeName, redemption, creating, error, onClose, onRedeem }: Props) {
  ...
  return <div className="fixed inset-0 z-[60] flex items-end bg-black/40" role="dialog" aria-modal="true" aria-label="Reward redemption">
    <div className="flex max-h-[92dvh] w-full flex-col overflow-hidden rounded-t-[2rem] bg-canvas shadow-2xl">
      ...
      {!status && <div className="shrink-0 border-t border-line bg-canvas px-5 pb-[calc(1rem+env(safe-area-inset-bottom))] pt-4"><button onClick={() => onRedeem(reward)} disabled={creating} className="primary-button w-full disabled:opacity-50">{creating ? 'Creating voucher…' : isPointsReward ? 'Redeem Now' : 'Claim Offer'}</button><p className="mt-2 text-center text-xs text-muted">Points are only used after cafe staff verifies this reward.</p></div>}
    </div>
  </div>;
}
