...
export function RewardRedemptionSheet({ reward, cafeName, redemption, creating, error, onClose, onRedeem }: Props) {
  ...
  return <div className="fixed inset-0 z-50 flex items-end bg-black/40" role="dialog" aria-modal="true" aria-label="Reward redemption">
    <div className="max-h-[92vh] w-full overflow-y-auto rounded-t-[2rem] bg-canvas px-5 pb-8 pt-5 shadow-2xl">
      <div className="mx-auto mb-4 h-1.5 w-10 rounded-full bg-coffee-200" />
      ...
      {!status && <div className="mt-5 space-y-3"><p className="type-body text-muted">{reward.description}</p><p className="type-small text-coffee-700">{cafeName}</p>{isPointsReward && <p className="type-body font-semibold">{reward.pointsRequired} Points</p>}{promotion && <div className="rounded-2xl bg-coffee-100 p-3 text-sm text-coffee-700">{promotion}</div>}{error && <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button onClick={() => onRedeem(reward)} disabled={creating} className="primary-button mt-3 w-full disabled:opacity-50">{creating ? 'Creating voucher…' : isPointsReward ? 'Redeem Now' : 'Claim Offer'}</button><p className="text-center text-xs text-muted">Points are only used after cafe staff verifies this reward.</p></div>}
      {redemption && <div className="mt-5 text-center"><h3 className="type-section-title">{redemption.rewardTitle}</h3><p className="mt-1 text-sm text-muted">{cafeName}</p>{redemption.pointsRequired > 0 && <p className="mt-2 text-sm font-semibold">{redemption.pointsRequired} points {status === 'completed' ? 'used' : ''}</p>}{status === 'pending' && !isExpired && <><div className="mx-auto mt-5 w-fit rounded-2xl bg-white p-4"><QRCode value={qrValue} size={180} /></div><p className="mt-4 text-sm text-muted">Show this QR code to cafe staff</p><div className="mt-4 rounded-2xl bg-coffee-100 p-4"><span className="eyebrow">Code</span><p className="mt-1 text-3xl font-bold tracking-[.22em]">{redemption.redemptionCode}</p></div><div className="mt-3 flex items-center justify-center gap-2 text-sm font-semibold text-coffee-700"><Clock3 className="size-4" />Expires in: <Countdown expiresAt={redemption.expiresAt} onExpired={() => setLocallyExpired(true)} /></div><p className="mt-3 text-xs text-muted">READY TO USE — this reward has not been permanently redeemed yet.</p></>}{status === 'completed' && <CheckCircle2 className="mx-auto mt-5 size-14 text-green-600" />}{(isExpired || status === 'rejected') && <Gift className="mx-auto mt-5 size-14 text-coffee-400" />}</div>}
    </div>
  </div>;
}
