...
function ReviewCard({ review }: { review: CustomerReview }) {
  ...
  return (
    <figure className="flex h-[166px] w-[248px] shrink-0 flex-col items-start rounded-3xl border border-[#e9dfd2] bg-white px-5 py-4 text-left shadow-[0_14px_30px_rgba(74,49,35,0.06)]">
      <div className="text-[13px] leading-none tracking-[0.1em] text-[#c68642]" aria-label={`${review.rating} out of 5 stars`}>{stars}</div>
      <blockquote className="mt-3 line-clamp-2 text-[14px] leading-[1.4] text-[#5c4033]">“{review.review}”</blockquote>
      <figcaption className="mt-auto flex items-center gap-2.5 pt-3">
        <div aria-hidden="true" className="flex size-[34px] items-center justify-center rounded-full bg-[#f3e9dc] text-sm font-semibold text-[#7a4f2a] ring-2 ring-[#f8f0e6]">{initials}</div>
        <div className="min-w-0 flex-1"><div className="flex items-center gap-1.5"><p className="truncate text-[13px] font-semibold text-neutral-950">{review.name}</p>{review.verified && <BadgeCheck className="size-3.5 shrink-0 text-emerald-600" aria-label="Verified review" />}</div></div>
      </figcaption>
    </figure>
  );
}
...
export function CustomerReviewsMarquee({ reviews, rating, reviewCount }: { reviews: CustomerReview[]; rating: number; reviewCount: number }) {
